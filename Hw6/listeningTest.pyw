...
INSTRUCTIONS =  "This is a test tool to evaluate the quality of a perceptual audio "\
...
class ITUR(wx.Frame):
    ...
    def instructionsClick(self, event=None):
        dlg = wx.MessageBox(INSTRUCTIONS, 'Instructions', 
...
class TestPanel(wx.Panel):
    ...
    def writeHeader(self):
        ...
        self.fgs.AddMany([(wx.Frame(self)), (buttonA), (buttonB), (buttonRef),
                      (trialNo), (gradeA), (gradeB), (wx.Frame(self))])
                                            
    def playFile(self, option):
        if not self.parent.session_manager.currentTrial(): return
        sound = wx.adv.Sound( self.parent.session_manager.currentTrial().getFileName(option) )
        sound.Stop()
        #sound.Play(wx.SOUND_SYNC)
        sound.Play(wx.adv.SOUND_ASYNC)

    def playA(self, event=None): self.playFile('a')
    def playB(self, event=None): self.playFile('b')
...
class SettingsPanel(wx.Panel):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        fgs.AddMany([(numSessions), (self.numSessionsCtrl, 1, wx.EXPAND),
                    (logFile, 1, wx.EXPAND), (self.logTextCtrl, 1, wx.EXPAND),
                    (playRef), (self.chkRef)])
    ...
    def enableControls(self, enable=True):
        if enable: self.numSessionsCtrl.Disable()
        else: self.numSessionsCtrl.Disable()
...
class Trial(object):
    ...
    def isCompleted(self):
        ...
        try:
            ...
            msg = "Trial %s is not complete: one of the grades is not a number! " \
    ...
    def getFileName(self, option): # option == 'a' or 'b'
        if option.lower() == self.original: return self.reference_file
        else: return self.test_file
...
class Session(object):
    ...
    def nextTrial(self, auto_play_reference=False):
        if self.current_trial >= 0 and not self.trials[self.current_trial].isCompleted(): return
        self.current_trial += 1
...
class SessionManager(object):
    ...
    def addSession(self, session_number): 
        ...
        self.test_panel.newSession( session_number, self.num_sessions) # HACK! (need to decouple this)
        random.shuffle(self.test_files) # randomize the test files order
        for f in self.test_files: session.addTrial(self.reference, f)
        # setup the UI
        self.test_panel.clearTrials()
    ...
    def useFolder(self, selected="./input.wav"):
        ...
        for name in os.listdir(folder_name):
            if name[-4:].lower() == ".wav" :
                if name == file_name: self.reference = os.path.join(folder_name, name)
                else: self.test_files.append(os.path.join(folder_name, name))
        self.nextSession()
    
    def currentIndex(self): return self.cur_indx
    
    def nextTrial(self):
        if self.cur_indx >= self.num_sessions or self.cur_indx == -1: return
        session = self.sessions[self.cur_indx]
        session.nextTrial(self.parent.autoPlayReference()) # HACK! (need to decouple this)
        if session.isCompleted(): self.nextSession()
    ...
    def nextSession(self): 
        ...
        if self.cur_indx < self.num_sessions:
            ...
            dlg = wx.MessageBox("You will now start a new %s session" % (TEST_TITLE), 'Info', wx.OK | wx.ICON_INFORMATION)
            session = self.sessions[self.cur_indx]
            session.nextTrial(self.parent.autoPlayReference()) # HACK! (need to decouple this)
        else:
    ...
    def summarize(self):
        summary = {}
        for f in self.test_files: summary[f] = {}
        ...
        f = open(self.parent.getLogFileName()+"_"+\
                  os.path.basename(self.reference)[:-4]+".csv",\
                  'a+') # HACK! (need to decouple this)
        f.write(output)
        f.close()
